...
import bdad.etl.{Scenarios, util}
...
object Main extends App {
  ...
  def autocorrelation(): Unit = {
    ...
    new TLCCModel(gassesIngress, gassesIngress, Array(-90, -60, -30, 10, 0, 10, 30, 60, 90)).allPlayAll()
    cors.saveAsTextFile("gasses-2014-2019-autocorrelation.txt")
  }

  /**
    * Computes the correlation between the Criteria Gasses and
    * Petroleum for 2019. This is done with lag, and correlations
  ...
  def correlation(): Unit = {
    ...
    val cors: RDD[((String, String, Int), Double)] =
      new TLCCModel(gassesIngress, petroleumIngress, Array(-60, -30, -10, 0, 10, 30, 60)).allPlayAll()
  ...
  /**
    * Computes the correlation between the Criteria Gasses and
    * Petroleum for 2014-2019. This is done with lag, and correlations
    * are printed to console, and written to file.
    */
  def correlation2014to2019(): Unit = {
    // Computes only on 2019 data for example purposes
    val (gasses, gas_labels) = Scenarios.gasses2014to2019()
    val (petroleum, petroleum_labels) = Scenarios.petroleum2014to2019()

    val gassesIngress = new TLCCIngress(gasses, gas_labels, yearCol = "year", dayCol = "dayofyear")
    val petroleumIngress = new TLCCIngress(petroleum, petroleum_labels)

    // Computes cross-correlation between un-shifted signals of gas and petroleum
    val cors: RDD[((String, String, Int), Double)] =
      new TLCCModel(gassesIngress, petroleumIngress, Array(-60, -30, -10, 0, 10, 30, 60)).allPlayAll()
    cors.saveAsTextFile("gasses-petroleumprice-corr-2014-2019.txt")
    cors.collect.foreach(println)
  }
  ...
  def correlationToxics(): Unit = {
    ...
    // Computes cross-correlation between un-shifted signals of gas and petroleum
    val cors: RDD[((String, String, Int), Double)] =
      new TLCCModel(toxicsIngress, petroleumIngress, Array(-30, -10, 0, 10, 30)).allPlayAll()
  ...
  /**
    ...
    * are printed to console, and written to file.
    */
  def correlationToxics2014to2019(): Unit = {
    val (toxics, toxicsLabels) = Scenarios.toxics2014to2019()
    val (petroleum, petroleum_labels) = Scenarios.petroleum2014to2019()

    val toxicsIngress = new TLCCIngress(toxics, toxicsLabels, yearCol = "year", dayCol = "dayofyear")
    val petroleumIngress = new TLCCIngress(petroleum, petroleum_labels)

    // Computes cross-correlation between un-shifted signals of gas and petroleum
    val cors: RDD[((String, String, Int), Double)] =
      new TLCCModel(toxicsIngress, petroleumIngress, Array(-30, -10, 0, 10, 30)).allPlayAll()
    cors.saveAsTextFile("toxics-petroleumprice-corr-2014-2019.txt")
    cors.collect.foreach(println)
  }
  ...
  correlation()
  correlationToxics()
  correlation2014to2019()
  // correlationToxics2014to2019() // BUG: Some names of the toxics introduce invalid characters
  autocorrelation()
